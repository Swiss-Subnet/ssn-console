...
activate-user:
    #!/usr/bin/env bash
    set -euo pipefail
    json=$(icp canister call backend list_user_profiles '(record {})' -e local --json)
    ...
    id=${line%%$'\t'*}
    icp canister call backend update_user_profile "(record { user_id = \"$id\"; status = opt variant { Active } })" -e local
