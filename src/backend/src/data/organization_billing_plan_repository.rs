...
pub fn set_plan(org_id: OrgId, plan: OrganizationBillingPlan) {
    mutate_state(|s| {
        s.billing_plans.insert(org_id, plan);
    });
}

pub fn get_plan(org_id: OrgId) -> Option<OrganizationBillingPlan> {
...
pub fn get_or_default(org_id: OrgId) -> OrganizationBillingPlan {
    get_plan(org_id).unwrap_or_default()
}

pub fn delete_plan(org_id: OrgId) {
    mutate_state(|s| {
