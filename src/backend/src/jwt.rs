...
fn invalid(message: &str) -> RejectionError {
    RejectionError::new(message.to_string(), RejectionReason::Invalid)
}
...
pub fn verify_jwt<T: DeserializeOwned>(
    ...
    if parts.len() != 3 {
        return Err(invalid("Invalid JWT token format"));
    }

    let message = format!("{}.{}", parts[0], parts[1]);
    let sig_bytes = URL_SAFE_NO_PAD
        .decode(parts[2])
        .map_err(|_| invalid("Invalid JWT signature base64 encoding"))?;

    let signature =
        Signature::from_slice(&sig_bytes).map_err(|_| invalid("Invalid JWT signature length"))?;
    let verifying_key = VerifyingKey::from_bytes(public_key_bytes)
        .map_err(|_| invalid("Invalid JWT public key"))?;

    verifying_key
        .verify(message.as_bytes(), &signature)
        .map_err(|_| invalid("JWT signature verification failed"))?;

    let payload_bytes = URL_SAFE_NO_PAD
        .decode(parts[1])
        .map_err(|_| invalid("Invalid JWT payload base64 encoding"))?;
    let base_claims: BaseClaims = serde_json::from_slice(&payload_bytes)
        .map_err(|_| invalid("Failed to parse JWT base claims"))?;
    ...
    let claims: T = serde_json::from_slice(&payload_bytes)
        .map_err(|_| invalid("Failed to parse JSON claims"))?;
