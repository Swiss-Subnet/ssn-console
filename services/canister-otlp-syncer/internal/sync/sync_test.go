...
func snapshot(id principal.Principal, ts, burned uint64) cmgen.CyclesMetricsSnapshotDto {
	return cmgen.CyclesMetricsSnapshotDto{
		TimestampNs:                    ts,
		CanisterId:                     id,
...
type page struct {
	snapshots []cmgen.CyclesMetricsSnapshotDto
	next      *cmgen.Cursor
}

type fakeCanister struct {
	pages    []page
	calls    []cmgen.Cursor // cursor seen on each ListMetricsAfter call
	recorded []backendgen.CanisterUsage
}

func (f *fakeCanister) ListMetricsAfter(_ context.Context, cursor *cmgen.Cursor) ([]cmgen.CyclesMetricsSnapshotDto, *cmgen.Cursor, error) {
	f.calls = append(f.calls, *cursor)
	p := f.pages[len(f.calls)-1]
	return p.snapshots, p.next, nil
}

func (f *fakeCanister) RecordUsage(_ context.Context, usages []backendgen.CanisterUsage) error {
...
func cursorAt(ts uint64) *cmgen.Cursor {
	return &cmgen.Cursor{Field0: ts, Field1: minPrincipal()}
}
...
func TestRunPagesAndDedupsLatestUsage(t *testing.T) {
	...
	client := &fakeCanister{pages: []page{
		...
		{snapshots: []cmgen.CyclesMetricsSnapshotDto{snapshot(a, 100, 11), snapshot(b, 110, 21)}, next: cursorAt(110)},
		// page 2: a@200 (newer -> wins), then nil cursor ends the loop
		{snapshots: []cmgen.CyclesMetricsSnapshotDto{snapshot(a, 200, 99)}, next: nil},
	}}
	pusher := &fakePusher{}
	dir := t.TempDir()
	...
	if client.calls[1].Field0 != 110 {
		t.Errorf("page 2 cursor ts = %d, want 110", client.calls[1].Field0)
	}
	if pusher.pushed != 2 {
...
func TestRunRestoresAndPersistsTimestamp(t *testing.T) {
	...
	client := &fakeCanister{pages: []page{
		{snapshots: []cmgen.CyclesMetricsSnapshotDto{snapshot(canisterID(1), 600, 1)}, next: cursorAt(600)},
	...
	if client.calls[0].Field0 != 500 {
		t.Errorf("start cursor ts = %d, want 500 (restored)", client.calls[0].Field0)
	}
...
func TestRunDefaultsToOneHourAgoWhenNoFile(t *testing.T) {
	...
	if client.calls[0].Field0 != want {
		t.Errorf("start cursor ts = %d, want %d (now-1h)", client.calls[0].Field0, want)
	}
}
